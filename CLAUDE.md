...
| `docs/dogfooding_scale_and_security.md` | DOGFOOD REPORT (2026-06-07) | 公開 Python リポジトリ (litellm / langgraph / pdm) の実 merged commit を対象に 3 sub-pass を実行: (1) 大規模/巨大関数 scale pass (`check`, randomized constraint seed 20260607, 5 件)、 (2) random-sampling extractor robustness pass (generic 0-constraint, 5 件)、 (3) security pass (SSP SAST=Semgrep + SCA=pip-audit, 実 merged-then-fixed 脆弱性 2 件含む 5 件)。 16 runs で 0 crash / 0 timeout / 100% well-formed JSON (langgraph 89 files・514-symbol surface delta、 litellm/proxy +5951 LOC 含む)、 CodeState cache cold 103s → warm 11s。 pattern SAST は実 SSRF / pricing-injection logic vuln を 0 findings で systematically miss → Phase H (LLM scout layer) motivation を empirical 検証。 SCA auto-discovery が PEP 621 pyproject / pdm.lock 非認識で unknown 退化 → **D8**(未解決, fixable defect)、 SAST blindspot → F6 (observation only)。 D-class 状態は `docs/dogfooding_findings_tracker.md` に集約 |
