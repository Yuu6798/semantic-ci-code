...
2. `--from-pr-body` / `--from-labels` / `--from-commits` / `--from-issue` の
   いずれかが指定された
3. `--add-api` / `--test-case` / `--remove-api` / `--allow-fqn` /
