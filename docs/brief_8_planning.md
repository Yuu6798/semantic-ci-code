...
3. **authoring hazard を Advisor 化**: D1 / D3 / D4 + positive expectation
   欠落 + severity downgrade 等を `target-doctor` で検出、verdict は変えない
   (exit code 常に 0、`--strict-advice` は **実装しない**)。D5 は PR #65
...
|---|---|---|
| `feature:add-api` | **以下のいずれか必須**: `--add-api FQN+` / `--from-pr-body` の `## Expected public API` セクションから FQN+ 抽出。optional `--test-case FQN*` | `api_surface_delta.added includes_all [{fqn: <FQN1>, visibility: "public"}, {fqn: <FQN2>, visibility: "public"}, …]`(record match で **`visibility: "public"` を強制**、平坦投影 alias `api_surface_delta.added.fqns` を使うと visibility が捨てられて non-public 追加でも satisfy されるため不可、`framework/match_schema.py:47` の `_API_SCHEMA.optional_keys={"kind", "visibility"}` を活用)、`--add-api` を逐語保持して PR body 抽出と union dedup merge、`--test-case` 指定時は `test_surface_delta.new_cases includes_all [<test FQN…>]`、未指定時は test_surface 制約なし |
| `bugfix:regression-test` | optional `--test-case FQN*` | `--test-case` 指定時 `test_surface_delta.new_cases includes_all [<test FQN…>]`、未指定時 `test_surface_delta.new_cases not_equals []`。template により `api_surface_public equals_baseline`(public API は **追加も削除も不可**、`compiler/templates.py:64-69`)+ `effect_changes.added equals ()`(新規 effect 不可、`compiler/templates.py:70-75`)を auto-expand。public API 変更を伴う修正には別 recipe(`feature:add-api` 等)を使う |
...
| `test-update:add-test-case` | optional `--test-case FQN*` | `--test-case` 指定時 `test_surface_delta.new_cases includes_all [<test FQN…>]`、未指定時 `test_surface_delta.new_cases not_equals []`。`primary_kind=test_update` で template `api_surface_public equals_baseline` + `effect_changes equals {added: (), removed: ()}` + `imports equals_baseline` が auto-expand(`compiler/templates.py:91-108`、test 追加以外を全 lock)。プロダクトコード変更を伴う場合は別 recipe |
...
- **C4**: PR body / issue の **intent-declaring structured section**(下記
  ...
  |---|---|
  | `## Expected public API` | `feature:add-api` |
  | `## Removed public API` | (本 brief の 4 recipe では消費されない、
    将来 brief で `feature:remove-api` 等を追加する余地) |
  | `## Test cases` / `## Acceptance Criteria` | 各 recipe の `--test-case`
    と同等(`feature:add-api` / `bugfix:regression-test` /
    `test-update:add-test-case` で消費可) |
  ...
  registry に **ある**が消費 recipe が無い section は error。
...
後に positive expectation の FQN list が空かどうかを検証**する。具体的には:

1. CLI parse: `--add-api` を **optional** として受け取る
2. PR body / issue parse: structured section から FQN list を抽出
3. merge: user input(strong)を base に、PR body(strong)→ issue
   (medium)の順に **union 追加**(重複は dedup)
...
flow が成立する(`docs/target_authoring_surface.md` の AI assistant 経由
authoring シナリオ)。CSCI-42 acceptance に当該 flow の golden fixture
を含める。
...
遵守)。generation_metadata の自動記録は **以下のいずれかが発火した時のみ**:

1. `--recipe <ID>` が指定された
2. `--from-pr-body` / `--from-labels` / `--from-commits` / `--from-issue` の
   いずれかが指定された
3. `--add-api` / `--test-case` / `--allow-fqn` /
   `--allow-fqn-prefix` / `--declared-at` のいずれかが指定された

上記いずれも未指定なら **generation_metadata block を一切書かず**、`init`
の出力は現行と byte-identical でなければならない。CSCI-42 acceptance に
「plain `init`(引数なし)出力が `tests/cli/test_init_command.py` の既存
assertion を pass」「recipe / source mode で **のみ** generation_metadata
が生成される」を明記。
...
- [ ] **plain `init`(引数なし)出力に generation_metadata は含まれない**
      (recipe / source / explicit-input flag のいずれかが発火した時のみ
      block 生成、§5.2 Provenance metadata スコープ厳守ルール)
