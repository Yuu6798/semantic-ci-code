...
| ID | タスク | Executor | 規模 |
|---|---|---|---|
| **W1-a** | docs 整合 sweep: S6 (stale D-class 記述の 8/8 化。「5 of 7」「未解決」「both open」等の stale 表現を repo 全体 grep で検出 — 確認済み残存: ROADMAP.md:41 / README.md:28・155 / `code_semantic_ci_design.md:1295` / `dogfooding_real_pr_complexity.md` ×4。原則 = D-class の現在 status 主張は tracker 単独に集約し、他 doc は cross-link + 解決 PR 番号へ置換) + S7 (doc_refactor self-archive — 列挙更新ではなく **repo-wide inbound-link grep を必須**: 現確認 13 file、`CLAUDE.md` 表 / README / `AGENTS.md` / `tests/discipline/README.md` / `test_index_md_entry_compactness.py` / `ssp_protocol_design_note.md` 等の live doc・test は新 path/文言へ更新、dated memory log と archive log は歴史記録として不変。broken link ゼロで完了) + S8 (open questions へ解決/据置を追記 + `CLAUDE.md` Design Documents 表・README planning list の完走済 Phase G/H 行を現状化 + 完走済 phase の planning doc **本文**の内部矛盾も解消 — 例: `llm_sensor_adapter_planning.md` は冒頭で H-1〜H-5 完走を宣言しつつ直後の前提節が「G-5 残・G-5 完走を前提・STATUS 投入は今後判断」のまま) + `brief_8_planning.md §15.1` grounding bullet 追加 + §2.4 の 3 件の declared-asymmetry pin (S3/S4/S5) | **Sonnet** | 半日 / docs のみ |
...
| ID | タスク | Executor | 規模 |
|---|---|---|---|
| **W2-a (CSCI-57)** | **Ghost-facet advisory (S1)**: extractor 未実装 facet を target する constraint への ADVISORY-D9 (仮)。scope は state path (`type_relations` / `control_flow` / `data_flow` / `coverage`) と**対応する delta path (`type_changes` / `cfg_delta` / `coverage_delta` 等) の両方** — constraint は delta 側も target できるため片側だけの警告は穴になる。error ではなく advisory (§23.1 pin、§2.2)。未実装 facet 集合 (state↔delta 対応含む) は extractor registry + path_schema から導出し hardcode しない (extractor 追加で自動解消)。**template-origin 対応**: doctor は template 合成後の compiled target を見るため、stock refactor template の `template:refactor:type_relations_unchanged` (`compiler/templates.py:48`) が既に ghost facet を target している — 放置すると全 default refactor target で user が書いていない constraint に D9 が鳴る。pin = D9 は **user-authored constraint に限定** (template/user の origin 区別は D3 の既存機構を再利用) し、template 側は同 brief 内で「更新 / 削除 / template-origin 警告の明示 surfacing」のいずれかで確定する — **rationale 記載のみ (挙動不変・advisory なし) は closure と認めない**: それでは default refactor target に不活性 hard constraint が無警告で残り続ける。推奨 = template から ghost-facet constraint を除去し、hand-built state 利用者には明示 authoring での宣言を案内 (この template 自体が S1 の実害の実例)。advisory enum 拡張は使用中 enum への追加ゆえ **`advisory-3` bump 必須** (D6/D7 追加時の advisory-1→2 と同一の compatibility policy。`doctor_advisory.schema.json` / `docs/json_schema.md` / schema sync test の同期更新を AC に含める — bump なしで D9 を emit すると strict consumer の JSON validation が割れる) | **Opus** | 1 日 / authoring + doctor + test |
