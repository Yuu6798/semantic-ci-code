...
- **クロスモデル自動 dedup**: モデル A と B が同じ穴を見つけると同じ anchor に
  潰れる → canonical_id 一意性 validator (Phase G SensorState validator 6) が
  自動併合。recall は N モデル分広がるがノイズは増えない
...
### H-5: target.yaml 昇格経路ドキュメント + クロスモデル dedup 検証 (CSCI-54)
- scout → authoring freeze → 制約化の経路を `docs/` に明文化 (D8)
- 複数 LLM adapter の anchor 自動 dedup を検証 (§1.5)
- **AC**: 2 つの LLM adapter が同一脆弱性を別 prose で報告 → 同一 canonical_id
  に併合される regression test
