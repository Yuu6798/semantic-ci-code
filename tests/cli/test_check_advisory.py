...
def test_check_advisory_rejects_unsupported_formats_and_bad_flags(tmp_path: Path):
    ...
    assert "cross-model aggregation is H-5 scope" in multiple.stderr
...
def test_check_advisory_does_not_change_verdict_summary_or_exit_code(tmp_path: Path):
    ...
    with_advisory = _run_check_with_advisory(repo, advisory_payload)
...
def _write_mutes(path: Path, *, canonical_id: str, expires: str) -> Path:
    path.write_text(
        "\n".join(
            (
                "mutes:",
