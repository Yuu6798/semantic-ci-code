...
from ._helpers import REPO_ROOT, extract_section

STATUS_MD = REPO_ROOT / ".claude" / "memory" / "STATUS.md"
FIXTURES = Path(__file__).parent / "fixtures"
NEXT_QUEUE_HEADING = "次の発行順序"
COMPLETION_MARKERS = ("完走", "landed")
NARRATIVE_SUBSECTIONS = ("### Sequencing decisions", "### 直近最短経路")
...
def _has_completion_marker(line: str) -> bool:
    lower = line.lower()
    return any(marker in line or marker in lower for marker in COMPLETION_MARKERS)
...
def _assert_no_completed_queue_items(text: str, *, source: str) -> None:
    ...
    raise AssertionError(
        f"{source} `## 次の発行順序` contains completed queue markers:\n"
        f"{details}\n"
        "Suggested action: CLAUDE.md rule says closed CSCI / Brief / D# items "
        "must move to 直近 merged and be removed from 次の発行順序."
    )
