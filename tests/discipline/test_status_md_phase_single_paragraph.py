...
from ._helpers import REPO_ROOT, extract_section, split_paragraphs
...
def _phase_paragraphs(text: str) -> list[str]:
    return split_paragraphs(extract_section(text, "Phase"))
...
def _assert_single_phase_paragraph(text: str, *, source: str) -> None:
    ...
    raise AssertionError(
        f"{source} `## Phase` must be exactly one paragraph; "
